<template>
    <article class="max-w-[900px] mx-auto px-4 py-10">
      <h1 class="text-3xl font-semibold">{{ doc.title || 'Privacy Policy' }}</h1>
...
//  definePageMeta({ layout: 'admin', middleware: 'auth' })
  ...
  const { data } = await useAsyncData('privacy', () => fetchPage('privacy'))
  const doc = computed(() => data.value?.content || {})
  
  // make relative /uploads URLs absolute (simple pass)
  const html = (s) => (s || '').replaceAll('src="/uploads', `src="${origin}/uploads`)
  useSeoMeta({ title: doc.value?.title || 'Privacy Policy' })
  </script>
